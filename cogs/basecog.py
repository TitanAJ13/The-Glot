...
class BaseCog(commands.Cog):
    ...
    async def verify_admin(self, ctx : commands.Context, id: int, email : str):
        ...
        if check not in user.roles:
            ...
            role = discord.utils.find(lambda r:r.name == section, self.bot.all_roles)
            print(f"Role type: {role}")

            roles = user.roles

            if (self.bot.t1 in roles and self.bot.t1 != role):
                print("removing t1")
                await user.remove_roles(self.bot.t1,reason="Verification")
            if (self.bot.t2 in roles and self.bot.t2 != role):
                print("removing t2")
                await user.remove_roles(self.bot.t2,reason="Verification")
            if (self.bot.bari in roles and self.bot.bari != role):
                print("removing bari")
                await user.remove_roles(self.bot.bari,reason="Verification")
            if (self.bot.bass in roles and self.bot.bass != role):
                print("removing bass")
                await user.remove_roles(self.bot.bass,reason="Verification")
